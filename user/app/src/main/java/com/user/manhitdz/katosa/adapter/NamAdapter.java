...
public class NamAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder>{
    Context context;
    ...
    public NamAdapter(Context context, List<SanPhamMoi> array) {
    ...
    public RecyclerView.ViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        if(viewType == VIEW_TYPE_DATA){
            View view = LayoutInflater.from(parent.getContext()).inflate(R.layout.item_donam, parent, false);
    ...
    public void onBindViewHolder(@NonNull RecyclerView.ViewHolder holder, int position) {
        if(holder instanceof MyViewHolder){
            ...
            if (!sanPham.getHinhanh().contains("http")){
                url = Utils.BASE_URL+"images/"+sanPham.getHinhanh();
            }else {
                url = sanPham.getHinhanh();
            }
            Glide.with(context).load(url).into(myViewHolder.hinhanh);
