...
import com.user.manhitdz.katosa.Interface.ImageClickListenner;
import com.user.manhitdz.katosa.R;
import com.user.manhitdz.katosa.model.EventBus.TinhTongEvent;
...
public class GioHangAdapter extends RecyclerView.Adapter<GioHangAdapter.MyViewHolder> {
    ...
    public void onBindViewHolder(@NonNull MyViewHolder holder, int position) {
        ...
        if (!gioHang.getHinhanh().contains("http")){
            url = Utils.BASE_URL+"images/"+gioHang.getHinhanh();
        }else {
            url = gioHang.getHinhanh();
        }
        holder.txtsize.setText("Size: "+gioHang.getSize());
        holder.txtcolor.setText(gioHang.getColor());
        Glide.with(context).load(url).into(holder.item_giohang_image);
    ...
    public class MyViewHolder extends RecyclerView.ViewHolder implements View.OnClickListener {
        ...
        TextView item_giohang_tensp, item_giohang_gia, item_giohang_soluong, item_giohang_giasp2, txtcolor, txtsize;
        ...
        public void onClick(View view) {
            if(view == item_giohang_tru){
                listenner.onImageClick(view, getAdapterPosition(), 1); // 1 là trừ
            }else if(view == item_giohang_cong){
                listenner.onImageClick(view, getAdapterPosition(), 2); // 2 là cộng
            }
        }
    }
}